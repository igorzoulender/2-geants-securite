    ...
    <v-container fluid>
        <div>
                    ...
                    <v-card>
                        ...
                        <v-card-text>
                            <v-container>
                                <v-row>
                                    <v-col cols="12">
                                        <v-text-field v-model="publication.title" label="Intitulé*"
                                            placeholder="Tapez l'intitulé du cour" required
                                            :rules="nameRules"></v-text-field>
                                    </v-col>
                                    <v-col cols="12">

                                        <vue-quill-editor :rule="nameRules" id="editor" useCustomImageHandler
                                            @imageAdded="handleImageAdded" v-model="publication.subject">
                                        </vue-quill-editor>
                                    </v-col>
                                </v-row>
                            </v-container>
                        ...
                        <v-card-actions>
                            ...
                            <v-btn v-if="publicationIsEditing" color="blue darken-1" class="tw-text-white"
                                :loading="registerBtn" @click="updateCour()">
                                Mettre à jour
                            </v-btn>
                        </v-card-actions>
                    </v-card>
            ...
            <div
                    ...
                    <div class="tw-flex tw-flex-wrap tw-items-center">
                        <div class="tw-relative tw-w-full tw-px-4 tw-max-w-full tw-flex tw-flex-1">
                            <h3 class="tw-font-semibold tw-text-lg tw-whitespace-nowrap">
                                Paremètre généraux du site
                            </h3>
                            <div class="tw-flex tw-justify-end tw-w-full">
                                <p class="tw-text-green-600 tw-text-lg hover:tw-cursor-pointer">
                                    <v-icon class="tw-text-green-600"
                                        @click="uploadAdresseModal()">mdi-plus-circle-outline</v-icon>
                                    Adresse
                                </p>
                            </div>
                        </div>
                            ...
                            <tr v-for="(publication, index) in displayedData" :key="index">
                                <th
                                    ...
                                    <img :src="backendImage(publication.photo)"
                                        class="tw-h-12 tw-w-12 tw-bg-white tw-rounded-full tw-border"
                                ...
                                <td
                                    ...
                                    <a href="#" class="tw-text-gray-800">{{ publication.created_at | moment('DD-MM-YYYY')
                                    }}</a>
                                </td>
                                ...
                                <td style="width: 60px"
                                    ...
                                    <div class="tw-flex tw-items-center tw-gap-2">
                                        <v-btn @click="statusPublication(publication)">
                                            <v-icon v-if="publication.status"
                                                color="green">mdi-lock-open-variant-outline</v-icon>
             ...
             <!-- modal pour affecter des stars au dep -->
             <v-dialog v-model="modal.uplaodAddresse" persistent max-width="700px">
                <v-card>
                    <v-toolbar dark color="tw-bg-yellow-600">
                        <v-btn icon @click.native="modal.uplaodAddresse = false" dark>
                            <v-icon>mdi-close</v-icon>
                        </v-btn>
                        <v-toolbar-title>Choose image to uplaod</v-toolbar-title>
                        <v-spacer></v-spacer>
                    </v-toolbar>
                    ...
                    <v-form>
                        ...
                        <v-card-actions>
                            <!-- <v-checkbox label="Post" v-model="post"></v-checkbox> -->
                            <v-spacer></v-spacer>

                            <v-btn color="tw-bg-yellow-600" :loading="loading" :disabled="!post"
                                @click="uploadImageSave()"><span class="tw-text-white">Save</span></v-btn>
                        </v-card-actions>
                    </v-form>
                </v-card>
            </v-dialog>
...
export default {
    ...
    methods: {
        ...
        async uploadAdresseModal() {
            this.modal.uplaodAddresse = true;
        },
