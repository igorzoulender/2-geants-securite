      ...
      <div
        ...
        <div class="">
          <v-form ref="PublicationRef" lazy-validation>
            ...
            <v-btn @click="updatePublication()" :loading="PublicationBtn" color="#1A1943"
              class="tw-text-white tw-w-full tw-mt-6">Mettre à jour
            </v-btn>
          </v-form>
        </div>
      </div>
  ...
  methods: {
    ...
    async updatePublication() {
      if (this.$refs.PublicationRef.validate()) {
        ...
        formData.append("subject", this.publication.subject);
        formData.append("photo", this.publication.photo);
        await axios.post('/blog-publication-update/' + this.$route.params.id, formData).then(res => {
          if (res.data.error) {
            Swal.fire({
              title: 'Echec',
              text: "Une erreur s'est produite",
          ...
          this.publication = {
            ...
            this.showToast('success', 'Publication mis à jour avec succès')
          this.$router.replace('/admin/publications')
        }).catch(error => {
          this.PublicationBtn = false
          console.log(error)
          this.showToast('error', "Error")
